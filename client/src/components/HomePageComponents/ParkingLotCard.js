...
const ParkingLotCard = ({ vehicleType, startTime,type, endTime, name, noOfFreeSlots, charges, distance, id, freeSlots, engagedSlots, address, lat, lng, currLoc, lotImages }) => {
    ...
    PaperProps={{
        style: {
            maxHeight: '90vh', // Limits the dialog height to 90% of the viewport height
            overflow: 'auto' // Enables scrolling within the dialog
        }
    }}
    sx={{ ...styles.dialog, padding: 2 }}
>
                ...
                <Grid container item xs={12} spacing={2}>
                    {[...freeSlots, ...engagedSlots].sort().map((slot) => (
                        <Grid item xs={3} key={slot}>
                            {engagedSlots.includes(slot) ? (
                                <ParkingSlot prevChanged={prevChanged} setPrevChanged={setPrevChanged} setChanged={setChanged} changed={changed} id={slot} booked />
                            ) : (
                                <ParkingSlot prevChanged={prevChanged} setPrevChanged={setPrevChanged} setChanged={setChanged} changed={changed} id={slot} booked={false} />
                            )}
                        </Grid>
                    ))}
                </Grid>
                ...
                <form autoComplete="off" noValidate sx={styles.form} onSubmit={handleConfirmSlot}>
                    <Grid container sx={styles.dialog} justifyContent="center">
                        ...
                        <Grid item sm={12} xs={12} sx={styles.ipFields}>
                            ...
                            <FormHelperText children="*Only jpg/jpeg/png file allowed to upload" />
                        </Grid>
